	...
	return {
		...
		compile: function(tplele,tplattr,transclude){
			if(tplele === 1){
				...
				canvas.setAttribute('progress-mprogressodel',node.getAttribute('progress-model'));
				...
				var icradius = node.getAttribute('progerss-inner-circle-radius') || '50';
				...
				return{
					pre:function preLink(scope,instanceElement,instanceAttribute,controller){
						...
						scope.$watch(expression,function(newValue,oldValue){
							...
							var endAngle = ((Math.PI*2) * newValue.precentage) - (Math.PI*2);
